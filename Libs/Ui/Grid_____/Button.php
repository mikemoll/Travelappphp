...
class Ui_Grid_Button
{
	...
	public function setDisplay($value, $name)
	{
		$this->value = $value;
        $this->name = $name;
	}
	...
	public function render()
	{
		$buttons = '';
		if($this->visible)
			$buttons = "{name: '{$this->name}', value: '{$this->value}', onpress: '{$this->onpress}',url: '{$this->url}',rel: '{$this->rel}', event: '{$this->event}', sendFormFields: '{$this->sendFormFields}', href: '{$this->href}', img: '{$this->img}', attribs: '{$this->attribs}' }";
		return $buttons;
	}
}